...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.sliders.update', $slider->id) }}" method="POST" enctype="multipart/form-data" 
          x-data="{ imageType: 'file', previewUrl: '{{ $slider->image_url }}' }" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div x-show="imageType === 'url'" class="space-y-2">
                <input type="url" name="image_url" value="{{ old('image_url', $slider->image) }}" 
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 text-xs font-bold">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $slider->title) }}" class="w-full px-4 py-2.5 bg-slate-50 border rounded-xl">
                </div>
                <div>
                    <label class="block text-slate-700 mb-1">برچسب ویژه (Tag Badge)</label>
                    <input type="text" name="tag" value="{{ old('tag', $slider->tag) }}" class="w-full px-4 py-2.5 bg-slate-50 border rounded-xl">
                </div>
            </div>

            <div>
                <label class="block text-slate-700 mb-1">زیرعنوان / توضیح</label>
                <input type="text" name="subtitle" value="{{ old('subtitle', $slider->subtitle) }}" class="w-full px-4 py-2.5 bg-slate-50 border rounded-xl">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-slate-700 mb-1">متن دکمه</label>
                    <input type="text" name="button_text" value="{{ old('button_text', $slider->button_text) }}" class="w-full px-4 py-2.5 bg-slate-50 border rounded-xl">
                </div>
                <div>
                    <label class="block text-slate-700 mb-1">لینک هدف</label>
                    <input type="text" name="link" value="{{ old('link', $slider->link) }}" class="w-full px-4 py-2.5 bg-slate-50 border rounded-xl font-mono dir-ltr text-left">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex items-center justify-between text-xs font-bold">
            <div class="flex items-center gap-6">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $slider->sort_order) }}" class="w-24 px-3 py-2 bg-slate-50 border rounded-xl font-mono text-center">
                </div>
                <label class="flex items-center gap-2 cursor-pointer mt-5">
                    <input type="checkbox" name="is_active" value="1" {{ $slider->is_active ? 'checked' : '' }} class="w-4 h-4 rounded text-[#ef394e]">
