...
<div x-show="mobileMenuOpen" 
    ...
    <div>
        ...
        <div class="p-5 border-b border-slate-800 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="{{ asset('images/logo/VAREN_WHITE_WITHOUT%20LOGOTYPE%20AND%20SLOGAN.png') }}" alt="VAREN" class="h-8 w-8 object-contain">
                <div>
                    <span class="text-sm font-black text-white block">پنل مدیریت وارن</span>
                    <span class="text-[10px] text-slate-400 font-bold">{{ auth()->user()->name }}</span>
                </div>
            </div>
    ...
    <div class="p-4 border-t border-slate-800 space-y-2">
        <a href="{{ route('home') }}" target="_blank" class="flex items-center gap-2.5 px-4 py-3 text-xs font-bold text-slate-300 hover:text-white transition rounded-xl bg-slate-800/80">
