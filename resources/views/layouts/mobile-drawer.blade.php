...
<div x-show="mobileDrawerOpen" 
    ...
    <div @click.away="mobileDrawerOpen = false" 
        ...
        <div class="p-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
            <img src="{{ asset('images/logo/VAREN_BLACK_WITH%20LOGOTYPE%20AND%20SLOGAN.png') }}" alt="وارن" class="h-8 w-auto">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3">
            <a href="{{ route('home') }}" class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-zinc-50 text-zinc-800">
                <i class="fa-solid fa-house text-zinc-400"></i>
                <span>صفحه نخست</span>
            </a>

            <a href="{{ route('home', ['category' => 'custom']) }}" class="flex items-center justify-between p-3 rounded-2xl bg-indigo-50 border border-indigo-100 text-indigo-950">
            ...
            <a href="{{ route('home', ['category' => 'hoodie']) }}" class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-zinc-50 text-zinc-800">
                <i class="fa-solid fa-snowflake text-zinc-400"></i>
                <span>هودی و دورس زمستانه</span>
            </a>

            <a href="{{ route('home') }}" class="flex items-center gap-3 p-2.5 rounded-xl text-rose-600 bg-rose-50/60 font-black">
            ...
            <div x-data="{ exp: false }" class="space-y-1">
                ...
                <div x-show="exp" class="pr-5 pl-1 space-y-1 pt-1" style="display: none;">
                    @foreach($navCategories as $cat)
                        <a href="{{ route('home', ['category' => $cat->slug]) }}" class="block p-1.5 font-bold text-zinc-800">{{ $cat->name }}</a>
                        @foreach($cat->children as $sub)
                            <a href="{{ route('home', ['category' => $sub->slug]) }}" class="block pr-3 py-1 text-[11px] text-zinc-500">- {{ $sub->name }}</a>
                        @endforeach
                    @endforeach
                </div>
            </div>
            ...
            <a href="{{ route('account.orders') }}" class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-zinc-50 text-zinc-700">
