...
@section('account_breadcrumb', 'داشبورد')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-l from-zinc-900 to-zinc-800 text-white rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-black mb-1">سلام، {{ $user->name }} عزیز خوش آمدید! 👋</h1>
            <p class="text-xs text-zinc-300 font-medium">از طریق این پنل می‌توانید سفارش‌ها و اطلاعات کاربری خود را مدیریت کنید.</p>
        </div>
        <a href="{{ route('home') }}" class="self-start sm:self-auto px-5 py-2.5 bg-white text-zinc-900 hover:bg-[#ef394e] hover:text-white font-black text-xs rounded-xl shadow transition duration-300">
            مشاهده فروشگاه
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl border border-zinc-200 p-4 shadow-sm flex items-center gap-3.5">
            ...
            <div>
                ...
                <span class="text-lg font-black text-zinc-900">{{ $totalOrders }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-zinc-200 p-4 shadow-sm flex items-center gap-3.5">
            ...
            <div>
                ...
                <span class="text-lg font-black text-zinc-900">{{ $processingCount }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-zinc-200 p-4 shadow-sm flex items-center gap-3.5">
            ...
            <div>
                ...
                <span class="text-lg font-black text-zinc-900">{{ $completedCount }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-zinc-200 p-4 shadow-sm flex items-center gap-3.5">
            ...
            <div>
                ...
                <span class="text-lg font-black text-zinc-900">{{ $pendingCount }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-zinc-200 p-6 shadow-sm">
        <div class="flex items-center justify-between pb-4 mb-4 border-b border-zinc-100">
            ...
            @if($recentOrders->isNotEmpty())
                <a href="{{ route('account.orders') }}" class="text-xs font-bold text-[#ef394e] hover:underline flex items-center gap-1">
        ...
        @if($recentOrders->isEmpty())
            <div class="text-center py-10">
                ...
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-black text-white hover:bg-[#ef394e] font-bold text-xs rounded-xl transition">
                    ...
                    <div class="border border-zinc-100 rounded-2xl p-4 hover:border-zinc-300 transition-colors bg-zinc-50/50">
                        <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-zinc-200/60 text-xs">
                            <div class="flex items-center gap-3">
                                <span class="font-black text-zinc-900 font-mono">#{{ $order->order_number }}</span>
                        ...
                        <div class="pt-3 flex flex-wrap items-center justify-between gap-4">
                            <div class="flex items-center gap-2 overflow-x-auto py-1">
                                @foreach($order->items as $item)
                                    <div class="w-12 h-14 rounded-xl bg-white border border-zinc-200 overflow-hidden shrink-0 flex items-center justify-center p-0.5" title="{{ $item->product_name }}">
                                ...
                                @endforeach
                                <span class="text-[11px] text-zinc-500 font-bold mr-2">{{ $order->items->count() }} کالا</span>
                            </div>

                            <div class="flex items-center gap-4">
                                <div class="text-left">
                                    <span class="text-[10px] text-zinc-400 block font-bold">مبلغ نهایی:</span>
                                    <span class="text-xs font-black text-zinc-900">{{ number_format($order->grand_total) }} تومان</span>
                                </div>
