...
<?php $__env->startSection('title', 'ویرایش محصول: ' . $product->name); ?>
...
<div class="max-w-4xl mx-auto bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm"
     x-data="{
         price: '<?php echo e(old('price', (int)$product->price)); ?>',
         salePrice: '<?php echo e(old('sale_price', $product->sale_price ? (int)$product->sale_price : '')); ?>',
         gallery: <?php echo e(json_encode(old('gallery_images', $galleryList))); ?>,
         selectedSizes: <?php echo e(json_encode(old('available_sizes', $existingSizesArray))); ?>,
         presetSizes: ['S', 'M', 'L', 'XL', '2XL', '3XL', 'Oversize', 'Free Size'],
         selectedColors: <?php echo e(json_encode(old('available_colors', $existingColorsArray))); ?>,
         presetColors: [
         ...
         attributes: <?php echo e(json_encode(old('attributes', $attrList))); ?>,
         toggleSize(sz) {
    ...
    <div class="flex items-center justify-between pb-4 mb-6 border-b border-slate-100">
        <div>
            ...
            <p class="text-xs text-slate-400">شناسه کالا: <?php echo e($product->sku); ?></p>
        </div>
        <a href="<?php echo e(route('admin.products.index')); ?>" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition">
            انصراف و بازگشت
        </a>
    </div>
    ...
    <form action="<?php echo e(route('admin.products.update', $product->id)); ?>" method="POST" class="space-y-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold">
            ...
            <div class="sm:col-span-2 flex flex-wrap items-center gap-6 p-4 bg-slate-50 rounded-2xl border border-slate-200/80">
                <label class="flex items-center gap-2.5 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" <?php echo e(old('is_active', $product->is_active) ? 'checked' : ''); ?> class="w-4 h-4 text-[#ef394e] rounded border-slate-300">
                    <span class="text-xs font-bold text-slate-800">محصول فعال و قابل نمایش در سایت باشد</span>
                </label>

                <label class="flex items-center gap-2.5 cursor-pointer">
                    <input type="checkbox" name="is_featured" value="1" <?php echo e(old('is_featured', $product->is_featured) ? 'checked' : ''); ?> class="w-4 h-4 text-[#ef394e] rounded border-slate-300">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="url" name="featured_image" value="<?php echo e(old('featured_image', $product->featured_image)); ?>" placeholder="https://..." required 
            ...
            <div class="sm:col-span-2">
                ...
                <input type="url" name="size_guide_image" value="<?php echo e(old('size_guide_image', $existingSizeGuide)); ?>" placeholder="https://.../size-guide.jpg" 
                       class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:border-slate-400 font-mono dir-ltr text-left">
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="description" rows="4" 
                          class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:border-slate-400 font-normal leading-relaxed"><?php echo e(old('description', $product->description)); ?></textarea>
            </div>
        </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/products/edit.blade.php ENDPATH**/ ?>