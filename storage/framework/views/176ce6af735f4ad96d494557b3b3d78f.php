...
<div class="lg:col-span-8 flex flex-col justify-between space-y-5"
     x-data="{
         selectedSize: '<?php echo e($sizesList[0] ?? 'L'); ?>',
         selectedColor: '<?php echo e($colorsList[0]['name'] ?? 'مشکی'); ?>',
         sizes: <?php echo e(json_encode($sizesList)); ?>,
         colors: <?php echo e(json_encode($colorsList)); ?>,
         showSizeGuide: false,
         buyBoxCopied: false,
         copyProductLink() {
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-zinc-100">
            <div class="flex items-center gap-2">
                <span class="px-3 py-1 bg-black text-white text-[10px] font-black tracking-wider uppercase rounded-full shadow-sm">
                    <?php echo e($product->category?->name ?? 'برند وارِن | VAREN'); ?>
            ...
            <div class="flex items-center gap-3 text-xs">
                ...
                <span class="text-zinc-400 font-mono text-[11px]">شناسه: <?php echo e($product->sku); ?></span>
            </div>
        </div>
        ...
        <?php if($product->attributes->count() > 0): ?>
            <div class="mb-5 space-y-2">
                ...
                <div class="flex flex-wrap gap-2">
                    <?php $__currentLoopData = $product->attributes->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="px-3.5 py-1.5 bg-zinc-100/90 border border-zinc-200/80 rounded-xl text-xs flex items-center gap-1.5 font-bold">
                            <span class="text-zinc-400"><?php echo e($attr->attribute_name); ?>:</span>
                            <span class="text-zinc-900 font-extrabold"><?php echo e($attr->attribute_value); ?></span>
                        </div>
                ...
                <div class="text-left">
                    <?php if($product->has_discount): ?>
                        <div class="flex items-center gap-2 justify-end mb-1">
                            <span class="text-xs text-zinc-400 line-through font-mono"><?php echo e(number_format($product->price)); ?> تومان</span>
                        ...
                        <div class="flex items-baseline gap-1.5 justify-end">
                            <span class="text-2xl sm:text-3xl font-black text-amber-400 tracking-tight"><?php echo e(number_format($product->sale_price)); ?></span>
                            <span class="text-xs font-bold text-zinc-300">تومان</span>
                        </div>
                    <?php else: ?>
                        <div class="flex items-baseline gap-1.5 justify-end">
                            <span class="text-2xl sm:text-3xl font-black text-white tracking-tight"><?php echo e(number_format($product->price)); ?></span>
                            <span class="text-xs font-bold text-zinc-300">تومان</span>
                        </div>
                    <?php endif; ?>
                </div>
    ...
    <div class="pt-2 border-t border-zinc-100">
        <?php if($product->isInStock()): ?>
            <form action="<?php echo e(route('cart.add', $product->id)); ?>" method="POST" class="space-y-4">
                <?php echo csrf_field(); ?>
                ...
                <div class="flex items-center justify-between text-[11px] font-bold pt-1">
                    <?php if($product->stock <= 5): ?>
                        <span class="text-amber-600 flex items-center gap-1.5">
                            ...
                            موجودی انبار محدود است (تنها <?php echo e($product->stock); ?> عدد باقیست)
                        </span>
                    <?php else: ?>
                    ...
                    <a href="<?php echo e(route('home', ['category' => 'custom'])); ?>" class="text-zinc-500 hover:text-black underline text-[11px]">
    ...
    <div x-show="showSizeGuide" 
        ...
        <div class="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl relative border border-zinc-200">
            ...
            <div class="overflow-hidden rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center justify-center min-h-[250px]">
                <?php if($sizeGuideUrl): ?>
                    <img src="<?php echo e($sizeGuideUrl); ?>" alt="راهنمای سایزبندی" class="w-full h-auto max-h-[70vh] object-contain rounded-2xl">
                <?php else: ?>
...
<?php /**PATH /var/www/html/resources/views/shop/partials/product_buy_box.blade.php ENDPATH**/ ?>