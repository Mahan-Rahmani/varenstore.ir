...
<?php $__env->startSection('title', 'مدیریت کدهای تخفیف'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <form action="<?php echo e(route('admin.coupons.index')); ?>" method="GET" class="flex items-center gap-2">
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="جستجوی کد تخفیف..." 
            ...
            <select name="type" onchange="this.form.submit()" class="px-3 py-2.5 text-xs bg-white border border-slate-200 rounded-2xl focus:outline-none font-bold">
                ...
                <option value="percentage" <?php echo e(request('type') === 'percentage' ? 'selected' : ''); ?>>درصدی</option>
        ...
        <a href="<?php echo e(route('admin.coupons.create')); ?>" class="px-5 py-2.5 bg-[#ef394e] hover:bg-rose-600 text-white font-bold text-xs rounded-2xl shadow-md transition flex items-center gap-2">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="p-4 font-bold text-slate-700">
                                <div>
                                    ...
                                    <span><?php echo e($coupon->min_order_amount ? number_format($coupon->min_order_amount) . ' تومان' : 'بدون شرط'); ?></span>
                                </div>
                                <?php if($coupon->max_discount_amount): ?>
                                    <div class="mt-0.5">
                                        <span class="text-slate-400 text-[10px] block">سقف:</span>
                                        <span class="text-rose-600"><?php echo e(number_format($coupon->max_discount_amount)); ?> تومان</span>
                                    </div>
                                <?php endif; ?>
                            </td>
                            <td class="p-4 font-bold">
                                <span class="text-slate-900 font-black"><?php echo e($coupon->used_count); ?></span>
                                <span class="text-slate-400">/ <?php echo e($coupon->usage_limit ?? 'نامحدود'); ?></span>
                            </td>
                            <td class="p-4">
                                <?php if($coupon->expires_at): ?>
                                    <span class="dir-ltr text-right block font-mono text-[11px] <?php echo e($isExpired ? 'text-rose-600 font-bold' : 'text-slate-600'); ?>">
                                        <?php echo e($coupon->expires_at->format('Y/m/d H:i')); ?>
                            ...
                            <td class="p-4">
                                <form action="<?php echo e(route('admin.coupons.toggle', $coupon->id)); ?>" method="POST">
                                    ...
                                    <button type="submit" class="px-2.5 py-1 rounded-xl text-[11px] font-bold transition <?php echo e($coupon->is_active ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'); ?>">
                                        <?php echo e($coupon->is_active ? 'فعال' : 'غیرفعال'); ?>
                            ...
                            <td class="p-4 text-left">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('admin.coupons.edit', $coupon->id)); ?>" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded-xl transition">
                                        ویرایش
                                    </a>
                                    <form action="<?php echo e(route('admin.coupons.destroy', $coupon->id)); ?>" method="POST" class="inline" onsubmit="return confirm('آیا از حذف این کد تخفیف مطمئن هستید؟')">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/coupons/index.blade.php ENDPATH**/ ?>